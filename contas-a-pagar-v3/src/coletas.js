    ...
    if(!html.includes('financeiro-recebimento-addon')){
      ...
      (() => {
        ...
        function numeroLocal(v){
          ...
          let s=String(v).trim().split(' ').join('');
          if(!s)return null;
          if(s.includes(',')&&s.includes('.')){
            s=s.lastIndexOf(',')>s.lastIndexOf('.')?s.split('.').join('').replace(',','.'):s.split(',').join('')
          }else if(s.includes(',')){
        ...
        const fmtKg=v=>{
          ...
          return n===null?'—':new Intl.NumberFormat('pt-BR',{minimumFractionDigits:0,maximumFractionDigits:2}).format(n)+' kg'
        };
        const fmtBRL=v=>{
          const n=numeroLocal(v);

...
if(active==='dashboard'){
  ...
  const hdA={},hdC={};HA.forEach(o=>{const k=g(o,'Data')||'Sem data';hdA[k]??={valor:0,nomes:new Set()};hdA[k].valor+=num(g(o,'Valor'));const nome=String(g(o,'NOME')||'').trim();if(nome)hdA[k].nomes.add(nome)});HCf.forEach(o=>{const k=g(o,'Data')||'Sem data';hdC[k]??={valor:0,nomes:new Set()};hdC[k].valor+=num(g(o,'Valor'));const nome=String(g(o,'NOME')||'').trim();if(nome)hdC[k].nomes.add(nome)});const HKA=Object.keys(hdA).sort((a,b)=>(pd(a)||0)-(pd(b)||0)),HKC=Object.keys(hdC).sort((a,b)=>(pd(a)||0)-(pd(b)||0));bars('#helpersChart',HKA,HKA.map(k=>hdA[k].valor),HKA.map(k=>nf(hdA[k].nomes.size)+' ajud.'));bars('#checkersChart',HKC,HKC.map(k=>hdC[k].valor),HKC.map(k=>nf(hdC[k].nomes.size)+' conf.'));
}

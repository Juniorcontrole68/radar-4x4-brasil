...
if(active==='dashboard'){
  ...
  const hd={};H.forEach(o=>{const k=g(o,'Data')||'Sem data';hd[k]??={valor:0,nomes:new Set()};hd[k].valor+=num(g(o,'Valor'));const nome=String(g(o,'NOME')||'').trim();if(nome)hd[k].nomes.add(nome)});const HK=Object.keys(hd).sort((a,b)=>(pd(a)||0)-(pd(b)||0));bars('#helpersChart',HK,HK.map(k=>hd[k].valor),HK.map(k=>nf(hd[k].nomes.size)+' ajud.'));
}

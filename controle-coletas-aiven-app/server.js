...
async function duplicateColeta(id, novaData) {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(String(novaData || ''))) {
    const e = new Error('Nova data inválida.');
    e.status = 400;
    throw e;
  }
  ...
  `);
  if (!meta.rowCount) throw new Error('Estrutura da tabela de coletas não encontrada.');

  const originalResult = await pool.query('SELECT * FROM coletas WHERE id::text=$1 LIMIT 1', [id]);
  if (!originalResult.rowCount) {
    const e = new Error('Coleta não encontrada.');
    e.status = 404;
    throw e;
  }

  const original = originalResult.rows[0];
  const names = new Set(meta.rows.map(x => x.column_name));
  const dateCol = ['data_coleta','collection_date','data','date'].find(x => names.has(x));
  if (!dateCol) {
    const e = new Error('Não foi possível identificar o campo de data da coleta.');
    e.status = 500;
    throw e;
  }
  ...
  for (const m of meta.rows) {
    ...
    else if (col === 'data_recebimento') vals.push(null);
    else if (col === 'previsao_pagamento_fatura') vals.push(null);
    else vals.push(original[col]);
  }
  ...
  http.createServer(async (req, res) => {
    try {
      ...
      if (req.method === 'POST' && duplicarMatch) {
        ...
        try {
          const nova = await duplicateColeta(id, body.data);
...
start().catch(err => {
 ...
 + (i+1)).join(',');
  const inserted = await pool.query(
    `INSERT INTO coletas (${qCols}) VALUES (${placeholders}) RETURNING id::text AS id, "${dateCol.replace(/"/g,'""')}" AS data`,
    vals
  );
...
start().catch(err => {
 ...
 + (i + 1)).join(',');
  const safeDateCol = '"' + dateCol.replace(/"/g, '""') + '"';
  const sql = 'INSERT INTO coletas (' + qCols + ') VALUES (' + placeholders + ') RETURNING id::text AS id, ' + safeDateCol + ' AS data';
  const inserted = await pool.query(sql, vals);
  return inserted.rows[0];
}

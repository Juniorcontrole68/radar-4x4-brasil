...
const DELIVERY_FLEET=[
  {id:'fiorino',label:'Fiorino',kg:600,cost:400},
  {id:'van_furgao',label:'Van Furgão',kg:1500,cost:700},
  {id:'van_bau',label:'Van Baú',kg:1500,cost:800}
];
...
async function buildDeliveryProgram(date='',force=false){
  ...
  for(const r of enriched){
    ...
    if(r.peso>1500){review.push({...r,reviewReason:'Carga excede 1.500 kg em uma única entrega'});continue}
    eligible.push({...r,serviceWeekday:cityRule?.weekdayLabel||deliveryProgramDowLabel(targetDow)})
  }
  const detailedEligible=await deliveryProgramEnrichSsw101(eligible,force);
  const detailedByCtrc=new Map(detailedEligible.map(x=>[normCtrc(x.ctrc)||String(x.ctrc||''),x]));
  for(const original of enriched){
    const d=detailedByCtrc.get(normCtrc(original.ctrc)||String(original.ctrc||''));
    if(d)Object.assign(original,d)
  }
  const geo=await deliveryProgramGeo(detailedEligible),geoOk=[];
...
async function probeSswRevenueMenu(){
  ...
  const urls=[
    ...
    'https://sistema.ssw.inf.br/bin/menu01?act=TRO&f2=AMR'
  ];
  const out=[];
  for(const url of urls){
    try{
      const rr=await fetch(url,{headers:{'User-Agent':'Mozilla/5.0 Chrome/120 Safari/537.36','Cookie':cookie(),'Referer':'https://sistema.ssw.inf.br/bin/menu01'},redirect:'manual',signal:AbortSignal.timeout(15000)});
      apply(rr.headers);const html=await rr.text(),plain=htmlText38(html),hits=[];
      const keywords=['FATUR','RECEITA','PERFORMANCE','CLIENTE','EMITENTE','COMERCIAL','RESULTADO','FRETE'];
      for(const kw of keywords){
        const re=new RegExp(kw,'ig');let m,n=0;
        while((m=re.exec(plain))&&n<12){hits.push({kw,context:plain.slice(Math.max(0,m.index-100),Math.min(plain.length,m.index+220))});n++}
      }
      const rawMatches=[];
      for(const m of html.matchAll(/.{0,220}(?:FATUR|RECEITA|PERFORMANCE|CLIENTE|EMITENTE|COMERCIAL|RESULTADO|FRETE).{0,320}/gi)){
        const raw=String(m[0]||'');
        rawMatches.push({
          text:htmlText38(raw).slice(0,420),
          options:[...new Set([...raw.matchAll(/(?:f3=|op(?:cao)?=)(\d{1,4})/gi)].map(x=>x[1]))],
          programs:[...new Set([...raw.matchAll(/ssw\d{3,6}/gi)].map(x=>x[0]))]
        });
        if(rawMatches.length>=40)break
      }
      out.push({url,status:rr.status,bytes:Buffer.byteLength(html),hits:hits.slice(0,50),rawMatches})
    }catch(e){out.push({url,error:String(e.message||e)})}
  }
  return{ok:true,pages:out}
}

async function probeSsw83(){
  if(!internalSswConfigured())return{ok:false,error:'Credenciais internas SSW não configuradas'};
  const jar=new Map();
  const apply=headers=>{const list=typeof headers.getSetCookie==='function'?headers.getSetCookie():(headers.get('set-cookie')?[headers.get('set-cookie')]:[]);for(const raw of list){const pair=String(raw).split(';')[0],i=pair.indexOf('=');if(i>0)jar.set(pair.slice(0,i).trim(),pair.slice(i+1).trim())}};
  const cookie=()=>[...jar.entries()].map(([k,v])=>k+'='+v).join('; ');
  ...
  r=await fetch('https://sistema.ssw.inf.br/bin/ssw0422',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded','User-Agent':'Mozilla/5.0 Chrome/120 Safari/537.36','Referer':'https://sistema.ssw.inf.br/bin/ssw0422','Cookie':cookie()},body:body.toString(),redirect:'manual',signal:AbortSignal.timeout(15000)});apply(r.headers);await r.text();
  if(!jar.has('token'))return{ok:false,error:'Login interno SSW não aceito'};
  r=await fetch('https://sistema.ssw.inf.br/bin/menu01?act=TRO&f2=AMR&f3=83',{headers:{'User-Agent':'Mozilla/5.0 Chrome/120 Safari/537.36','Cookie':cookie(),'Referer':'https://sistema.ssw.inf.br/bin/menu01'},redirect:'manual',signal:AbortSignal.timeout(15000)});apply(r.headers);
  const nav=await r.text(),prog=(nav.match(/ssw\d+/i)||[])[0]||'';
  if(!prog)return{ok:false,status:r.status,error:'Programa da opção 83 não identificado',navBytes:Buffer.byteLength(nav)};
  r=await fetch('https://sistema.ssw.inf.br/bin/'+prog,{headers:{'User-Agent':'Mozilla/5.0 Chrome/120 Safari/537.36','Cookie':cookie(),'Referer':'https://sistema.ssw.inf.br/bin/menu01'},redirect:'manual',signal:AbortSignal.timeout(15000)});apply(r.headers);
  const html=await r.text();
  const title=htmlText38((html.match(/<title[^>]*>([\s\S]*?)<\/title>/i)||[])[1]||'');
  const inputs=[...html.matchAll(/<input\b([^>]*)>/gi)].map(m=>{const a=m[1]||'';return{name:(a.match(/\bname=["']?([^"'\s>]+)/i)||[])[1]||'',type:(a.match(/\btype=["']?([^"'\s>]+)/i)||[])[1]||'',max:(a.match(/\bmaxlength=["']?([^"'\s>]+)/i)||[])[1]||''}}).filter(x=>x.name).slice(0,60);
  ...
  const rows=[...html.matchAll(/<tr\b[^>]*>([\s\S]*?)<\/tr>/gi)].slice(0,12).map(m=>[...m[1].matchAll(/<t[dh]\b[^>]*>([\s\S]*?)<\/t[dh]>/gi)].map(x=>htmlText38(x[1])).filter(Boolean)).filter(x=>x.length);
  const text=htmlText38(html).slice(0,1200);
  return{ok:true,option:83,prog,status:r.status,bytes:Buffer.byteLength(html),title,inputs,forms,rows,text}
}


function deliveryProgramSpecialClass(text){
  const n=norm38(String(text||'')).replace(/[^A-Z0-9]+/g,' ');
  const tube=/\bTUBOS?\b|\bTUBULACAO\b/.test(n);
  const water=/\bCAIXAS?\s+(?:D\s+|DE\s+)?AGUA\b|\bRESERVATORIOS?\b/.test(n);
  return tube&&water?'tubos_caixa_agua':(tube?'tubos':(water?'caixa_agua':'normal'))
}
function deliveryProgramSpecialLines(text){
  return String(text||'').split(/\r?\n/).map(x=>x.trim()).filter(Boolean).filter(line=>{
    const n=norm38(line).replace(/[^A-Z0-9]+/g,' ');
    return /\bTUBOS?\b|\bTUBULACAO\b|\bCAIXAS?\s+(?:D\s+|DE\s+)?AGUA\b|\bRESERVATORIOS?\b/.test(n)
  }).slice(0,20)
}
...
async function deliveryProgramFetchSsw101Detail(session,row,force=false){
  ...
  if(!ctrc)return null;
  const hit=SSW101_DETAIL_CACHE.get(key);
  if(!force&&hit&&Date.now()-hit.at<12*60*60*1000)return hit.value;
  const mm=ctrc.match(/^([A-Z]{3})0*(\d+)-\d$/);
  if(!mm)return null;
  try{
    ...
    let rr=await fetch('https://sistema.ssw.inf.br/bin/'+session.prog,{
      method:'POST',
      headers:{...session.hdr('https://sistema.ssw.inf.br/bin/'+session.prog),'Content-Type':'application/x-www-form-urlencoded'},
      body:params.toString(),redirect:'manual',signal:AbortSignal.timeout(20000)
    });
    ...
    try{
      ...
      const prodRes=await fetch('https://sistema.ssw.inf.br/bin/'+session.prog,{
        method:'POST',
        headers:{...session.hdr('https://sistema.ssw.inf.br/bin/'+session.prog),'Content-Type':'application/x-www-form-urlencoded'},
        body:prodParams.toString(),redirect:'manual',signal:AbortSignal.timeout(20000)
      });
      session.apply(prodRes.headers);
      const prodHtml=await prodRes.text();
      const webBody=(prodHtml.match(/name=web_body[^>]*value=["']([^"']+)["']/i)||[])[1]||'';
      if(webBody){
        const decoded=decodeURIComponent(webBody.replace(/&amp;/g,'&'));
        const am=decoded.match(/abrir\(['"]([^'"]+)['"],\s*['"]([^'"]+)['"],\s*(\d+),\s*(\d+),\s*['"]([^'"]*)['"]/i);
        if(am){
          const pu=new URL('/bin/ssw0424','https://sistema.ssw.inf.br');
          pu.searchParams.set('act',am[1]);pu.searchParams.set('filename',am[2]);pu.searchParams.set('path',am[5]||'');pu.searchParams.set('down',am[3]);pu.searchParams.set('nw',am[4]);
          const fileRes=await fetch(pu,{headers:session.hdr('https://sistema.ssw.inf.br/bin/'+session.prog),redirect:'manual',signal:AbortSignal.timeout(20000)});
          session.apply(fileRes.headers);
          const fileBuf=Buffer.from(await fileRes.arrayBuffer());
          productText=fileBuf.toString('latin1');
          products=deliveryProgramSpecialLines(productText)
        }
      }
    }catch(e){console.log('PROGRAMAÇÃO SSW101 PRODUTO '+ctrc+': '+String(e.message||e))}
    const combined=(tm?.[1]||'')+' '+(em?.[1]||'')+' '+productText;
    const value={
    ...
    SSW101_DETAIL_CACHE.set(key,{at:Date.now(),value});
    return value
  }catch(e){
    console.log('PROGRAMAÇÃO SSW101 '+ctrc+': '+String(e.message||e));
    return null
  }
}
async function deliveryProgramEnrichSsw101(rows,force=false){
  if(!rows.length)return rows;
  let session=null;
  try{session=await deliveryProgramCreateSsw101Session()}catch(e){
    console.log('PROGRAMAÇÃO SSW101 SESSÃO: '+String(e.message||e));
    return rows
  }
  let freight=0,special=0,queried=0;
  const out=[];
  for(const row of rows){
    const key=normCtrc(row.ctrc)||String(row.ctrc||'');
    const wasCached=!force&&SSW101_DETAIL_CACHE.has(key)&&Date.now()-SSW101_DETAIL_CACHE.get(key).at<12*60*60*1000;
    const detail=await deliveryProgramFetchSsw101Detail(session,row,force);
    if(detail){
      queried+=wasCached?0:1;
      const x={...row,...detail,frete:Number(detail.frete||row.frete||0)};
      if(x.frete>0)freight++;
      if(x.specialClass&&x.specialClass!=='normal')special++;
      out.push(x)
    }else out.push(row)
  }
  console.log('PROGRAMAÇÃO SSW101 ENRIQUECIMENTO: '+JSON.stringify({rows:rows.length,queried,freight,special,cached:rows.length-queried}));
  return out
}
...
async function probeSsw101Cte(ctrc){
  ...
  if(!mm)return{ok:false,error:'CTRC teste inválido'};
  const jar=new Map();
  const apply=headers=>{const list=typeof headers.getSetCookie==='function'?headers.getSetCookie():(headers.get('set-cookie')?[headers.get('set-cookie')]:[]);for(const raw of list){const pair=String(raw).split(';')[0],i=pair.indexOf('=');if(i>0)jar.set(pair.slice(0,i).trim(),pair.slice(i+1).trim())}};
  const cookie=()=>[...jar.entries()].map(([k,v])=>k+'='+v).join('; ');
  let rr=await fetch('https://sistema.ssw.inf.br/bin/ssw0422',{headers:{'User-Agent':'Mozilla/5.0 Chrome/120 Safari/537.36'},redirect:'manual',signal:AbortSignal.timeout(15000)});apply(rr.headers);
  ...
  const formHtml=await rr.text(),params=deliveryProgramFormParams(formHtml);
  params.set('t_ser_ctrc',mm[1]);params.set('t_nro_ctrc',mm[2]);params.set('act','P1');
  ...
  for(const term of terms){
    const p=norm38(html).indexOf(norm38(term));
    contexts[term]=p>=0?htmlText38(html.slice(Math.max(0,p-1200),Math.min(html.length,p+3500))):''
  }
...
function receitaFindHeader(headers,aliases){
  const hs=(headers||[]).map(h=>({raw:h,norm:normKey(h)}));
  for(const a of aliases){const na=normKey(a),hit=hs.find(h=>h.norm===na);if(hit)return hit.raw}
  for(const a of aliases){const na=normKey(a),hit=hs.find(h=>h.norm.includes(na));if(hit)return hit.raw}
  return''
}
function receitaDateValue(row){
  return pickField(row,'DATA','DATA EMISSAO','DT EMISSAO','EMISSAO','DATA FATURAMENTO','DT FATURAMENTO')
}
function receitaInRange(row,from,to){
  if(!from&&!to)return true;
  const raw=String(receitaDateValue(row)||'').trim();
  if(!raw)return true;
  let iso='';
  if(/^\d{4}-\d{2}-\d{2}/.test(raw))iso=raw.slice(0,10);
  else iso=brDateToIso(raw.slice(0,10));
  if(!iso)return true;
...
http.createServer(async(req,res)=>{try{const u=new URL(req.url,'http://x');if(u.pathname==='/health'){res.writeHead(200,{'Content-Type':'application/json'});return res.end(JSON.stringify({ok:true}))}if(req.method==='GET'&&u.pathname==='/'&&u.searchParams.get('ticket')){try{
  ...
  html=html.replace(/<script src="\/app\.js(?:\?[^"]*)?"><\/script>/,bootstrap+'<script src="/app.js?v=20260925tracking11"></script>');
  res.writeHead(200,{
...
if(u.pathname==='/api/bi2/baixas'){try{if(!dashboardHasAny(authUser,['ssw_saidas','evolucao','cidade_destino']))return dashboardDeny(res);const x=await buildBi2Baixas(u.searchParams.get('date')||'');res.writeHead(200,{'Content-Type':'application/json; charset=utf-8','Cache-Control':'no-store'});return res.end(JSON.stringify(x))}catch(e){res.writeHead(502,{'Content-Type':'application/json; charset=utf-8','Cache-Control':'no-store'});return res.end(JSON.stringify({ok:false,error:String(e.message||e)}))}}if(u.pathname==='/api/bi2/saidas-baixas'){try{
...
}catch(e){res.writeHead(502,{'Content-Type':'application/json; charset=utf-8','Cache-Control':'no-store'});return res.end(JSON.stringify({ok:false,error:String(e.message||e)}))}}if(u.pathname==='/api/bi2/receita'){try{
  if(!dashboardHasAny(authUser,['receita_ssw','financeiro','remetentes']))return dashboardDeny(res);
  ...
  try{
    ...
    html=html
      ...
      .replace('<div id="authGate" class="auth-gate">','<div id="authGate" class="auth-gate hide" style="display:none!important">')
      .replace('<div id="loading" class="loading">Carregando dados do Google Sheets…</div>','<div id="loading" class="loading hide" style="display:none!important"></div>')
      .replace('<script src="/app.js"></script>','<script src="/app.js?v=20260925tracking11"></script>');
    res.writeHead(200,{
      'Content-Type':'text/html; charset=utf-8',
      'Cache-Control':'no-store, no-cache, must-revalidate',
      'Pragma':'no-cache',
